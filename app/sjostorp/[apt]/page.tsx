...
export async function generateStaticParams() {
  return getEstateApartments('sjostorp').map((a) => ({ apt: a.slug }))
}

export async function generateMetadata({ params }: { params: Promise<Params> }): Promise<Metadata> {
  const { apt } = await params
  const a = getApartment('sjostorp', apt)
  if (!a) return {}
  return { title: a.name, description: a.description }
}

export default async function SjostorpAptPage({ params }: { params: Promise<Params> }) {
  const { apt } = await params
  const apartment = getApartment('sjostorp', apt)
  if (!apartment) notFound()

  const slug = `sjostorp/${apt}`
    ...
    <>
      ...
      <main className="flex flex-col flex-1">
        ...
        <div className="relative w-full h-dvh min-h-[560px] overflow-hidden">
          <Image src={apartment.coverPhoto} alt={apartment.name} fill className="object-cover" priority sizes="100vw" />
          <div
            aria-hidden
